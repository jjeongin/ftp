	...
	while (1) { // repeat
        ...
        if (buffer[0] == '!') { // client command
            ...
            if (strcmp(response, "200 PORT command successful.") == 0) { // if PORT successful
                ...
                recv(client_sd, &response, sizeof(response), 0);
                printf("%s\n", response);
                if (strcmp(response, "150 File status okay; about to open data connection.") == 0) { // if file check successful
                    // get filename
                    ...
                    if (pid == 0) {
                        ...
                        {
                            if (errno == EADDRINUSE) {
                                // port occupied, pick a different port
                                bzero((char *) &data_addr, sizeof(data_addr));
                                data_addr.sin_family = AF_INET;
                                data_addr.sin_port = htons((unsigned short) 0); // pick any available port
                                data_addr.sin_addr.s_addr = INADDR_ANY;
                                if (bind(data_sd, (struct sockaddr *) &data_addr, sizeof(data_addr)) < 0) {
                                    printf("Error: could not bind to process (%d) %s\n", errno, strerror(errno));
                                    exit(-1);
                                }
                                data_port = ntohs(data_addr.sin_port); // get newly assigned port
                                printf("Port occupied, new data port: %d\n", data_port);
                            }
                            else {
                                printf("Error: could not bind to process (%d) %s\n", errno, strerror(errno));
                                exit(-1);
                        ...
                        // upload file
                        send_file(fp, data_server_sd, copy_buffer);
                        fclose(fp);
                        // recieve output through data connection
                        bzero(response, sizeof(response));
